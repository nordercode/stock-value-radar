...
const companyFinancials = document.querySelector("#companyFinancials");
...
const metricDefs = [
  ...
  ["sma20", "SMA 20", "20 Perioden Durchschnitt in EUR", "Der SMA 20 zeigt den kurzfristigen Durchschnittskurs im gewählten Zeitraum. Er hilft, kurzfristiges Momentum einzuordnen."],
  ...
  ["sma50", "SMA 50", "50 Perioden Durchschnitt in EUR", "Der SMA 50 glättet mittelfristige Kursschwankungen im gewählten Zeitraum und zeigt den mittleren Trend."],
  ["dist50", "Abstand SMA 50", "Technischer Abstand", "Zeigt, ob der Kurs über oder unter dem mittelfristigen Trend liegt."],
  ["sma200", "SMA 200", "200 Perioden Durchschnitt in EUR", "Der SMA 200 ist eine langfristige Trendlinie im gewählten Zeitraum. Oberhalb ist der Trend oft stärker, darunter vorsichtiger."],
...
function financialRow(label, value, yoyValue, formatter = money) {
  if (!Number.isFinite(value)) return "";
  const yoyClass = Number.isFinite(yoyValue) && yoyValue >= 0 ? "positive" : "negative";
  const yoy = Number.isFinite(yoyValue) ? `<em class="${yoyClass}">(${signedPercent(yoyValue)} YoY)</em>` : "";
  return `<div><span>${label}</span><strong>${formatter(value)} ${yoy}</strong></div>`;
}

function renderCompanyFinancials(profile) {
  const data = profile?.financials;
  const year = data?.year || 2025;
  const rows = [
    financialRow(`Umsatz ${year}`, data?.revenue, data?.revenueYoy, (value) => `${compact(value)} €`),
    financialRow(`Gewinn ${year}`, data?.profit, data?.profitYoy, (value) => `${compact(value)} €`),
    financialRow(`EPS ${year}`, data?.eps, data?.epsYoy, money)
  ].filter(Boolean);

  companyFinancials.innerHTML = rows.length
    ? rows.join("")
    : `<p>Für Umsatz, Gewinn und EPS liegen über die öffentliche Quelle aktuell keine belastbaren Jahresdaten vor.</p>`;
}
...
function render(payload) {
  ...
  renderCompanyFinancials(payload.profile);
  verdictText.textContent = payload.verdict;
  scoreText.textContent = `${payload.score}/100`;
  scoreMarker.style.left = `${100 - payload.score}%`;
...
document.querySelector(".stock-head").addEventListener("click", (event) => {
  if (event.target.closest("a, button")) return;
  event.currentTarget.classList.toggle("is-flipped");
});

document.querySelector(".stock-head").addEventListener("keydown", (event) => {
  if (!["Enter", " "].includes(event.key)) return;
  event.preventDefault();
  event.currentTarget.classList.toggle("is-flipped");
});
