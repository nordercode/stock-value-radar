...
function verdict(score, fundamentalCount = 0) {
  if (fundamentalCount < 3) {
    if (score <= 35) return "Technisch attraktiv";
    if (score <= 65) return "Neutral";
    return "Technisch riskant";
  }
  if (score <= 24) return "Invest";
  if (score <= 44) return "Eher attraktiv";
  if (score <= 62) return "Neutral";
  if (score <= 78) return "Teuer";
  return "Don't invest";
}
...
function buildRationale({ score, metrics, fundamentalCount }) {
  ...
  if (Number.isFinite(metrics.distHigh)) {
    if (metrics.distHigh < -35) reasons.push(`Zum All-Time-High besteht ein Abstand von ${Math.abs(metrics.distHigh).toFixed(1)}%, was auf eine starke Korrektur hinweist.`);
    else if (metrics.distHigh > -8) reasons.push(`Die Aktie notiert nah am All-Time-High; das erhöht das Rückschlagrisiko.`);
  }
  ...
  if (Number.isFinite(metrics.annualVolatility) && metrics.annualVolatility > 55) {
    reasons.push(`Die Volatilität ist mit ${metrics.annualVolatility.toFixed(1)}% hoch; die Aktie bleibt riskant.`);
  }
...
async function stockPayload(query, selectedRange = "12m") {
  ...
  const sma20 = sma(technicalCloses, 20);
  const sma50 = sma(technicalCloses, 50);
  const sma200 = sma(technicalCloses, 200);
  const dist20 = pctDistance(current, sma20);
  const dist50 = pctDistance(current, sma50);
  const dist200 = pctDistance(current, sma200);
  const oneYearStart = closes[0];
  const yearlyReturn = pctDistance(current, oneYearStart);
  const analystUpside = pctDistance(targetMean, current);
  const high = Math.max(...technicalCloses);
  const low = Math.min(...closes);
  const distHigh = pctDistance(current, high);
  const distLow = pctDistance(current, low);
  const annualVolatility = volatility(technicalCloses.slice(-252));
  const drawdown = maxDrawdown(technicalCloses.slice(-252));
  const rsi14 = rsi(technicalCloses);
  ...
  const metrics = {
    ...
    yearlyReturn: formatMaybe(yearlyReturn),
    rangeHigh: formatMaybe(high),
  ...
  const score = weightedScore([
    ...
    { ...valueSignal(dist200, -10, 25), weight: 0.8 },
    { ...valueSignal(dist50, -6, 14), weight: 0.55 },
    { ...valueSignal(distHigh, -35, -4), weight: 0.45 },
    { ...valueSignal(rsi14, 34, 72), weight: 0.45 },
    { ...valueSignal(annualVolatility, 24, 78), weight: 0.35 },
  ...
  return {
    ...
    profile: {
      logo: logoUrl(symbol),
      exchange,
      marketCap: compactNumber(unwrap(summary.price?.marketCap) ?? quote.marketCap),
    ...
    changePercent: formatMaybe(quote.regularMarketChangePercent),
